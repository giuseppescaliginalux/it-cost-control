...
class Contract {
  ...
  generateForecastLedger(currentLedger = []) {
    ...
    if (bt.includes("FIXED RECURRING")) {
      ...
      while (currentCursor <= finalEnd) {
        ...
        const hasCoverage = currentLedger.some(l => {
          ...
          // MATCH ATOMICO: Immunità ai fusi orari
    ...
    if (bt.includes("PAY-AS-YOU-GO") && (pm.includes("MINIMUM") || pm.includes("CAPPED"))) {
      ...
      // 🟢 1. SANITIZATION: Vaporizziamo i vecchi record CALCULATED dalla memoria prima di calcolare
      const cleanLedger = currentLedger.filter(l => String(l.type).toUpperCase().trim() !== "CALCULATED");

      // 🟢 2. BURN-DOWN LOGIC: Ignoriamo i crediti. Il Gross Consumption è eroso solo da Actual, Debit e Forecast
      const actSum = cleanLedger.filter(l => String(l.type).toUpperCase().trim() === "ACTUAL").reduce((sum, l) => sum + (parseFloat(l.amount) || 0), 0);
      const dbSum = cleanLedger.filter(l => String(l.type).toUpperCase().trim() === "DEBIT").reduce((sum, l) => sum + Math.abs(parseFloat(l.amount) || 0), 0);
      const fwdSum = cleanLedger.filter(l => String(l.type).toUpperCase().trim() === "FORECAST").reduce((sum, l) => sum + (parseFloat(l.amount) || 0), 0);

      const remainingCommitment = Math.max(0, this.totalCommitment - (actSum + dbSum) - fwdSum);

      // Estrazione ultimo mese di validità effettiva
      ...
      // --- CICLO DI CONTEGGIO MESI RIMANENTI ---
      let intervalsRemaining = 0;
      while (tempCursor <= finalEnd) {
        const chunkStart = new Date(tempCursor.getTime());

        const hasForecastInPeriod = cleanLedger.some(l => {
          const type = String(l.type || "").toUpperCase().trim();
          if (type !== "FORECAST" || !l.startDate) return false;
          const d = new Date(l.startDate);
          // 🟢 3. TIMEZONE IMMUNITY: Confronto basato solo su Anno e Mese assoluti
          return d.getFullYear() === chunkStart.getFullYear() && d.getMonth() === chunkStart.getMonth();
        });
      ...
      // --- CICLO DI GENERAZIONE DEL FORECAST ---
      ...
      while (generateCursor <= finalEnd) {
        ...
        const hasManualForecast = cleanLedger.some(l => {
          const type = String(l.type || "").toUpperCase().trim();
          if (type !== "FORECAST" || !l.startDate) return false;
          const d = new Date(l.startDate);
          // 🟢 3. TIMEZONE IMMUNITY: Replicato sulla generazione
          return d.getFullYear() === chunkStart.getFullYear() && d.getMonth() === chunkStart.getMonth();
        });
