...
(function () {
  ...
  registry.push({
    ...
    fn: function (assert) {
      ...
      assert.equal(MASTER_FIELD_MAP["Asset ID"] !== undefined, true); // <-- AGGIUNTA
    }
  });
  ...
  registry.push({
    ...
    fn: function (assert) {
      ...
      assert.equal(exported.assetId, "AST-123"); // <-- AGGIUNTA
  ...
  registry.push({
    ...
    fn: function (assert) {
      // Aggiunto assetId al master
      const master = new MasterContract({ masterId: "MCT-LOOKUP-01", assetName: "Cloud Platform", assetId: "AST-999", supplier: "Google", billingChannel: "Reseller" });
      const child = new Contract({ contractId: "CTR-LOOKUP-01", assetName: "Vecchio Asset", assetId: "AST-OLD", supplier: "Vecchio Fornitore", billingChannel: "Vecchio Canale" });
      ...
      assert.equal(exportedChild.assetId, "AST-999"); // <-- AGGIUNTA (deve sovrascrivere "AST-OLD")
